...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        //选中按钮
//        self.seleteImage = [[UIImageView alloc]initWithFrame:CGRectMake(viewWidth-17-projectManagerSize.height, self.projectManagerLabel.frame.origin.y, projectManagerSize.height+2, projectManagerSize.height+2)];
//        [self addSubview:self.seleteImage];
        
        //对号按钮
        //self.seleteBtn = [[UIButton alloc]initWithFrame:CGRectMake(viewWidth-17-projectManagerSize.height, self.projectManagerLabel.frame.origin.y, projectManagerSize.height+2, projectManagerSize.height+2)];
        ...
        //[self.seleteBtn addTarget:self action:@selector(seletedBtn:) forControlEvents:UIControlEventTouchUpInside];
        ...
        self.placeLabel.frame = CGRectMake(viewWidth-30-placeLableSize.width-self.seleteBtn.frame.size.width, self.projectManagerLabel.frame.origin.y, placeLableSize.width, placeLableSize.height);
        [self.contentView addSubview:self.placeLabel];
        //公司名称
        self.nameLabel = [UILabel new];
        self.nameLabel.text = @"浙江建设工程有限公司";
...
//    if (self.delegate && [self.delegate respondsToSelector:@selector(seletedBtn:)])
//    {
//        [self.delegate performSelector:@selector(seletedBtn:) withObject:sender];
//    }
...
- (void)loadContent:(MbUserInfo*)data
{
    ...
    CGSize nameLableSize = [self.nameLabel.text sizeWithFont:self.nameLabel.font constrainedToSize:CGSizeMake(300, 300) lineBreakMode:NSLineBreakByWordWrapping];
    self.nameLabel.frame = CGRectMake(15, (viewHeight-109)/6-projectManagerSize.height-7, viewWidth-120, nameLableSize.height);
